    ...
    async def handle_power_control(call: ServiceCall) -> None:
        ...
        try:
            ...
            if successful_devices:
                _LOGGER.info("Power control successful: %s -> %s", successful_devices, power_state)
            if len(successful_devices) < len(devices):
                failed_devices = set(devices) - set(successful_devices)
                _LOGGER.warning("Power control partially failed for devices: %s", list(failed_devices))
